...
rule check_per_sample_mean_tpm_filtered:
    ...
    output:
        os.path.join(STRINGTIE_SUBDIR, "{sample}.assembled.mean_tpm_filtered.gtf")
    ...
    shell:
        """
        if [ ! -e {output} ]; then
            echo Expected mean TPM filtered sample file was not generated - {output} - check rule tx_condition_mean_tpm_filter
            exit 1
        else
            echo Success! Expected mean TPM filtered sample file was generate by rule tx_condition_mean_tpm_filter - {output}
            exit 0
        fi &> {log}
        """
...
rule get_novel_last_exons:
    ...
    input:
        novel_gtf = os.path.join(STRINGTIE_SUBDIR,
                                 "{sample}.assembled.mean_tpm_filtered.gtf"
                                 ),
...
rule merge_novel_filtered_by_condition:
    '''
    ...
    benchmark:
        os.path.join(BMARK_SUBDIR,
                     "merge_novel_filtered_by_condition.txt")

    log:
        os.path.join(LOG_SUBDIR,
                     "merge_novel_filtered_by_condition.log")
...
rule get_combined_quant_gtf:
    ...
    '''
    input:
        novel_gtf = rules.merge_novel_filtered_by_condition.output.gtf,
        ref_gtf = rules.filter_ref_gtf.output if config["filter_ref_gtf"] else GTF
